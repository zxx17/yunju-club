...
import com.zsyj.circle.server.dao.ShareMessageMapper;
import com.zsyj.circle.server.entity.po.ShareMessage;
import com.zsyj.circle.server.rpc.AuthUserServiceRpc;
import com.zsyj.circle.server.service.ShareMessageService;
import com.zsyj.circle.server.util.LoginUtil;
import com.zsyj.circle.server.websocket.ChickenSocket;
...
public class ShareMessageServiceImpl extends ServiceImpl<ShareMessageMapper, ShareMessage> implements ShareMessageService {
    ...
    public void reply(String fromId, String toId, Long targetId) {
        ...
        message.put("msg", String.format("%s 回复了你的评论，快来看看把！", authUserServiceRpc.getUserInfo(fromId).getNickName()));
        message.put("targetId", targetId);
        ShareMessage shareMessage = new ShareMessage();
        shareMessage.setFromId(fromId);
        shareMessage.setToId(toId);
